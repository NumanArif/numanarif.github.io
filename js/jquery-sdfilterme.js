...
;(function($) {
    ...
    $.sdFilterMe.buildLayout = function(el, options) {
        ...
        maxWidth = 300;//Math.max.apply(Math, widths);
        maxHeight = 550;//Math.max.apply(Math, heights);

options.css.border.width = 4;
        ...
        $outerWrapper.before($el.clone(true)).find('> .sdfm-inner-wrapper').on('click', function() {
            $('#' + outerWrapperId).prev('ul').trigger('fm.boxClicked', [$(this), $(this).index(), $(this).attr('data-order')]);
        });
        $el.remove();
...
jQuery(function($) {
    $('#sort-me').sdFilterMe({
    ...
    }).on('fm.boxClicked', function(e, box, position, order) {
        ...
        console.log('Box sort order is ' + order);
            ...
            var modalImg = document.getElementById("img01");
              ...
              modalImg.src = img.prop('src');;
              modalImg.style.width=img.width();
              modalImg.style.height=img.height();
